...
bool SurfaceComposerClient::getRequiresClientComposition() {
    bool supported = false;
    ComposerServiceAIDL::getComposerService()->getRequiresClientComposition(&supported);
    return supported;
}
//SPD:added for if sf uses client composition by song.tang 20230704 end

//SPD: add for sfcpupolicy by song.tang 20241120 start
void SurfaceComposerClient::setTransitionState(bool isBegin) {
    ComposerServiceAIDL::getComposerService()->setTransitionState(isBegin);
}
//SPD: add for sfcpupolicy by song.tang 20241120 end

//SPD:add for sf thread info by sifeng.tian 20231117 start
int SurfaceComposerClient::getSurfaceFlingerTid() {
    int tid = 0;
    ComposerServiceAIDL::getComposerService()->getSurfaceFlingerTid(&tid);
    return tid;
}
int SurfaceComposerClient::getRenderEnginTid() {
    int tid = 0;
    ComposerServiceAIDL::getComposerService()->getRenderEnginTid(&tid);
    return tid;
}
void SurfaceComposerClient::getGetHwcTids(std::vector<int>* pTids) {
    ComposerServiceAIDL::getComposerService()->getHwcTids(pTids);
}
